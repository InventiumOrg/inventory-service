...
// CreateBusinessMetrics creates business-specific metrics for the inventory service
func CreateBusinessMetrics() (*BusinessMetrics, error) {
	meter := otel.Meter("inventory-service-business")

	// Sale Unit Operations
	inventoryOperations, err := meter.Int64Counter(
		"inventory_operations_total",
		metric.WithDescription("Total number of inventory operations"),
	)
	if err != nil {
		return nil, err
	}

	inventoryCreated, err := meter.Int64Counter(
		"inventory_created_total",
		metric.WithDescription("Total number of inventory created"),
	)
	if err != nil {
		return nil, err
	}

	inventoryRetrievals, err := meter.Int64Counter(
		"inventory_retrievals_total",
		metric.WithDescription("Total number of inventory retrievals"),
	)
	if err != nil {
		return nil, err
	}
	...
	inventoryRequests, err := meter.Int64Counter(
		"inventory_list_requests_total",
		metric.WithDescription("Total number of inventory list requests"),
	)
	if err != nil {
		return nil, err
	}

	inventoryDeletes, err := meter.Int64Counter(
		"inventory_deletes_total",
		metric.WithDescription("Total number of inventory deletions"),
	)
	if err != nil {
		return nil, err
	}
	...
	activeInventoryItems, err := meter.Int64UpDownCounter(
		"active_inventory_count",
		metric.WithDescription("Current number of active inventory items"),
	)
	if err != nil {
		return nil, err
	}
	...
	inventoryUpdates, err := meter.Int64Counter(
		"inventory_updates_total",
		metric.WithDescription("Total number of inventory updates"),
	)
	if err != nil {
		return nil, err
	}

	// Location-based inventory metrics
	inventoryByLocation, err := meter.Int64Counter(
		"inventory_by_location_total",
		metric.WithDescription("Total inventory items by location"),
	)
	if err != nil {
		return nil, err
	}

	// Category-based inventory metrics
	inventoryByCategory, err := meter.Int64Counter(
		"inventory_by_category_total",
		metric.WithDescription("Total inventory items by category"),
	)
	if err != nil {
		return nil, err
	}
	...
	return &BusinessMetrics{
		...
		InventoryRetrievals:    inventoryRetrievals,
		InventoryListRequests:  inventoryRequests,
		InventoryDeletes:       inventoryDeletes,
...
type BusinessMetrics struct {
	InventoryOperations    metric.Int64Counter
	InventoryCreated       metric.Int64Counter
	InventoryRetrievals    metric.Int64Counter
	InventoryListRequests  metric.Int64Counter
	InventoryDeletes       metric.Int64Counter
	InventoryUpdates       metric.Int64Counter
	InventoryByLocation    metric.Int64Counter
	InventoryByCategory    metric.Int64Counter
	...
	ActiveInventoryItems   metric.Int64UpDownCounter
}
