...
type PrometheusMetrics struct {
	...
	InventoryItemsActive     prometheus.Gauge
...
func NewPrometheusMetrics(serviceName string) *PrometheusMetrics {
	metrics := &PrometheusMetrics{
		...
		InventoryOperationsTotal: prometheus.NewCounterVec(
			prometheus.CounterOpts{
		...
		InventoryItemsActive: prometheus.NewGauge(
			prometheus.GaugeOpts{
				Name: "inventory_items_active",
				Help: "Current number of active inventory items",
			},
		),
	...
	prometheus.MustRegister(
		...
		metrics.InventoryOperationsTotal,
		metrics.InventoryItemsActive,
...
func (m *PrometheusMetrics) PrometheusMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// Get route pattern (e.g., "/api/v1/inventory/:id" instead of "/api/v1/inventory/123")
		route := c.FullPath()
		if route == "" {
			route = "unknown"
		}
...
func (m *PrometheusMetrics) RecordInventoryOperation(operation, category, location string) {
	m.InventoryOperationsTotal.WithLabelValues(operation, category, location).Inc()
}
...
func (m *PrometheusMetrics) UpdateInventoryCount(count float64) {
	m.InventoryItemsActive.Set(count)
}
